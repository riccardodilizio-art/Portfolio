      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="text-center mb-16">
            <p className="text-blue-600 mb-4 tracking-wider">COMPETENZE</p>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {skillCategories.map((category, index) => (
              <motion.div
                ...
                className="group"
              >
                <div className="h-full p-8 bg-slate-50 hover:bg-white rounded-2xl transition-all hover:shadow-lg">
                  ...
                  <h3 className="text-xl text-slate-900 mb-4">{category.title}</h3>
                  
                  <div className="flex flex-wrap gap-2">
                    {category.skills.map((skill, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-white group-hover:bg-blue-50 text-slate-700 rounded-full text-sm transition-colors"
                      >
                        {skill}
                      </span>
                    ))}
